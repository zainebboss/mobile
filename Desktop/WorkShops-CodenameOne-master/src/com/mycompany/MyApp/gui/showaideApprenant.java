 ...
 * @author nidha
 */
public class showaideApprenant extends Form{
    Form current;
    public showaideApprenant(Resources res){  
        ...
        ArrayList<Aide> alSub = ServiceAide.getInstance().displaySubject();
        for(Aide c : alSub) {
    ...
    private void addButton(Image img, Aide c, Resources res){
...
//        System.out.println(c.toString());
        Label subjectTxt = new Label("Sujet : "+c.getSujet(),"NewTopLine2");
        Label problemeTxt = new Label("Probleme : "+c.getProbleme(),"NewTopLine2");
        Label mailTxt = new Label("Mail : "+c.getMail(),"NewTopLine2");
        ArrayList<Aidecom> aidecom = ServiceAidecom.getInstance().displayComById(c.getId());
        String com = "";
        for(Aidecom ac : aidecom) {
            com = com + ac.getCommaintre()+"\r\n"; 
        }
        Label commentaire = new Label("Commentaire : "+com,"NewTopLine2");
//        System.out.println(classe.toString());
   
        
       //Aide aide = ServiceAide.getInstance().detailsUser(c.getId());
     //System.out.println(aide.toString());
      //Label idTeacherTxt = new Label("Enseignant : "+aide.getSujet()+" "+aide.getProbleme(),"NewTopLine2"+aide.getMail());
...
//        lActions.setTextPosition(CENTER);
        ...
        lSupp.addPointerPressedListener(l -> {         
            if (ServiceAide.getInstance().deleteSubject(c.getId()))
                new showaideApprenant(res).show();   
            });
            lActions.addPointerPressedListener(l -> {          
                new Home().show();
            });
        cnt.add(BorderLayout.CENTER, BoxLayout.encloseY(BoxLayout.encloseX(subjectTxt),BoxLayout.encloseX(problemeTxt),BoxLayout.encloseX(mailTxt),BoxLayout.encloseX(commentaire),BoxLayout.encloseXRight(lSupp),BoxLayout.encloseXRight(lActions)));
        add(cnt);
    }
}
